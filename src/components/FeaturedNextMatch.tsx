...
export default function FeaturedNextMatch() {
  ...
  const Flag = ({ src, size = 32 }: { src: string; size?: number }) =>
    src ? <img src={src} alt="" width={size} height={Math.round(size * 0.75)} className="rounded-sm shadow" /> : null;
  ...
  // ── Upcoming: AI scoreline hero + win% + preview + social proof ──
  ...
  return (
    <section className="mx-auto mb-10 max-w-2xl px-4">
      <div className="overflow-hidden rounded-2xl border border-[#f0a500]/30 bg-gradient-to-br from-[#1a1a2e] to-[#0e0e0e]">
        {/* Header: label + lock-in countdown */}
        <div className="flex items-center justify-between border-b border-[#222] px-6 py-3">
          <span className="text-xs font-bold uppercase tracking-widest text-[#f0a500]">⚽ Next Match{m.stage ? ` · ${m.stage}` : ""}</span>
          <span className="font-mono text-sm font-bold tabular-nums text-white">⏳ {clock}</span>
        </div>

        {/* AI predicted scoreline — the curiosity hero */}
        <div className="px-6 pt-6 pb-5 text-center">
          {pred && (
            <div className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-[#f0a500]">🧠 AI Predicts</div>
          )}
          <div className="flex items-center justify-center gap-3 sm:gap-5">
            <div className="flex flex-1 flex-col items-center gap-1.5">
              <Flag src={m.homeFlag} size={40} />
              <span className="text-sm font-semibold text-white sm:text-base">{m.home}</span>
            </div>
            {pred ? (
              <div className="text-5xl font-extrabold tabular-nums text-[#f0a500] sm:text-6xl">
                {pred.scoreHome}<span className="mx-1 text-[#555]">-</span>{pred.scoreAway}
              </div>
            ) : (
              <div className="text-2xl font-bold text-[#666]">vs</div>
            )}
            <div className="flex flex-1 flex-col items-center gap-1.5">
              <Flag src={m.awayFlag} size={40} />
              <span className="text-sm font-semibold text-white sm:text-base">{m.away}</span>
            </div>
          </div>
          {pred && <div className="mt-3 text-sm text-[#aaa]">Agree? <b className="text-white">Make your call</b> and beat the AI.</div>}
        </div>
        ...
        {m.preview?.teaser && (
          <div className="border-t border-[#222] px-6 py-5">
            ...
            <p className="text-sm leading-relaxed text-[#ddd]">{m.preview.full ?? m.preview.teaser}</p>
            {m.preview.locked && (
              <Link href={matchUrl} className="mt-3 flex items-center gap-2.5 rounded-lg bg-[#0f0f0f] px-4 py-3 transition-colors hover:bg-[#161616]">
                <Lock className="h-4 w-4 shrink-0 text-[#f0a500]" />
                <span className="text-sm text-[#bbb]"><b className="text-white">Sign in free</b> to read the full preview &amp; what to watch →</span>
              </Link>
            )}
          </div>
        )}

        {/* Social proof (aggregate) + CTA */}
        <div className="flex flex-col items-center gap-3 border-t border-[#222] bg-black/30 px-6 py-5 text-center">
          {m.totalPlayers && m.totalPlayers > 0 && (
            <div className="flex items-center gap-1.5 text-sm text-[#aaa]">
              <Users className="h-4 w-4 text-[#f0a500]" />
              <span>Join <b className="text-white">{roundDown(m.totalPlayers)}</b> players competing this World Cup</span>
            </div>
          )}
          <Link href={matchUrl} className="rounded-lg bg-[#f0a500] px-8 py-3 text-sm font-bold text-black transition-colors hover:bg-[#d49500]">
